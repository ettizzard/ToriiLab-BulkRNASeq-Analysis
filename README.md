...
We do not need to repeat any steps with STAR and can re-use the alignment ```bam``` files generated for each sample with the Cufflinks software suite.

According to the [Cufflinks manual,](https://cole-trapnell-lab.github.io/cufflinks/manual/) the Cufflinks RNA-Seq workflow starting with pre-mapped reads goes Cufflinks -> Cuffcompare -> Cuffmerge -> Cuffquant -> Cuffdiff. In our case, only Cufflinks -> Cuffmerge -> Cuffdiff was reqired.


### Cufflinks
In order for Cufflinks to create output files with gene names, the [TAIR ```gff``` gene annotation file](https://www.arabidopsis.org/download/list?dir=Genes/TAIR10_genome_release/TAIR10_gff3) was accessed and downloaded. Cufflinks was then used to assemble transcripts with gene annotation data.
...
--output-dir /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/cufflinks/cufflinks/DMSO_S1_output/ \
--GTF /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/star/genome_files/RefSeq/genomic_fixed.gtf \
...
--num-threads 16 \
--library-type fr-firststrand \
--output-dir /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/cufflinks/cufflinks/Estradiol_S3_output/ \
--GTF /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/star/genome_files/RefSeq/genomic_fixed.gtf \
/projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/star/alignment_output/Estradiol_S3_output/Aligned.sortedByCoord.out.bam
```



<!-- #### Cuffcompare
Running cuffcompare aids in the analysis of transcript fragments generated by Cufflinks and is necessary for downstream differential gene expression.

```
$ cuffcompare -r /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/TAIR10_GFF3_genes.gff /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/cufflinks/cufflinks/DMSO_S1_output/transcripts.gtf /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/cufflinks/cufflinks/DMSO_S2_output/transcripts.gtf /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/cufflinks/cufflinks/DMSO_S3_output/transcripts.gtf /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/cufflinks/cufflinks/Estradiol_S1_output/transcripts.gtf /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/cufflinks/cufflinks/Estradiol_S2_output/transcripts.gtf /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/cufflinks/cufflinks/Estradiol_S3_output/transcripts.gtf
...
### Cuffmerge
Cuffmerge simply merges the 6 ```gtf``` files generated by Cufflinks. Now, we will run Cuffmerge on all sample transcript files. To do this, we need to make a ```txt``` file with the paths and names of all 6 transcript ```gtf``` files.

It was discovered after running Cufflinks that Cuffmerge will not merge ```gtf``` transcript files from Cufflinks output that was generated with an annotation file containing unassigned transcripts. Since these transcripts are not associated with specific genes, they can simply be removed for the purposes of this analysis. The resulting annotation file was then used to re-run Cufflinks and finally Cuffmerge.

##### Grep command to remove duplicated unassigned transcripts
```
$ grep -v 'unassigned_transcript' /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/star/genome_files/RefSeq/genomic_fixed_nodupes.gtf > tmpfile && mv tmpfile /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/star/genome_files/RefSeq/genomic_fixed_nodupes.gtf
```


##### Standard Cuffmerge command
```
$ cuffmerge \
-p 16 \
-g /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/star/genome_files/RefSeq/genomic_fixed_nodupes.gtf \
/projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/cufflinks/cuffmerge/manifest.txt 
```


<!-- 
#### Cuffquant
The Cuffquant command creates an output ```cxb``` file for each sample that contains expression level information. These files are then passed to Cuffdiff for differential gene expression analysis.

```
$ cuffquant \
-p 16 \
-library-type fr-firststrand \
-o /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/cufflinks/cuffquant/DMSO_S1_output/ \
/projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/cufflinks/merged_asm/merged.gt \
/projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/star/alignment_output/DMSO_S1_output/Aligned.sortedByCoord.out.bam
...
$ cuffquant \
-p 16 \
-library-type fr-firststrand \
-o /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/cufflinks/cuffquant/Estradiol_S3_output \
/projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/cufflinks/merged_asm/merged.gtf \
/projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/star/alignment_output/Estradiol_S3_output/Aligned.sortedByCoord.out.bam
``` -->

### Cuffdiff
Cuffdiff now takes the resulting merged ```gtf``` file from Cuffmerge as input to carry out diffferential gene expression of each sample per condition type.
...
-library-type fr-firststrand \
-L Estradiol,DMSO \
-b /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/star/genome_files/RefSeq/GCF_000001735.4_TAIR10.1_genomic.fasta \
-o /projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/cufflinks/cuffdiff/ \
/projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/cufflinks/cuffmerge/merged_asm/merged.gtf \
/projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/star/alignment_output/Estradiol_S1_output/Aligned.sortedByCoord.out.bam,/projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/star/alignment_output/Estradiol_S2_output/Aligned.sortedByCoord.out.bam,/projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/star/alignment_output/Estradiol_S3_output/Aligned.sortedByCoord.out.bam \
/projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/star/alignment_output/DMSO_S1_output/Aligned.sortedByCoord.out.bam,/projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/star/alignment_output/DMSO_S2_output/Aligned.sortedByCoord.out.bam,/projects/bgmp/tizzard/bioinfo/ToriiLab-BulkRNASeq-Analysis/star/alignment_output/DMSO_S3_output/Aligned.sortedByCoord.out.bam
```
